...
- **`images/`** — source token art (PNG/JPEG), gitignored since it's the raw, unoptimized library. Files are named by generic monster id (e.g. `beholder-death-tyrant.png`), not the Monster Manual name, so art assets never need to move when a monster is renamed or recategorized — see `js/tokens.json` below.
...
- **`js/tokens.json`** — the hand-maintained pregenerated-token registry: one entry per token id, holding its display `name`, 5e creature `category` (used by the vault's category filter), and free-text search `tags` (used by the vault's search box). This is the file to edit when adding a token, renaming one, or fixing its category/tags.
- **`js/vault-data.js`** — auto-generated from `js/tokens.json` plus whichever image folders are available. One entry per monster: `{ id, name, category, tags, filenames }`, where `filenames` maps each style in `VAULT_STYLES` to that monster's art file for the style, or `null` if it hasn't been generated for that style yet. Do not edit by hand.
